/**
 * Merkle tree construction and proof generation for the ZK Signer registry.
 *
...
export function buildSignersTree(signers: SignerEntry[]): SignersTree {
    const leaves: bigint[] = signers.map(({ signer }) =>
        computeSignerLeaf(signer)
    );
 ...
 *
 * @throws if the signer is not found in the tree.
 */
export function getSignerMerkleProof(
    tree: LeanIMT,
    signer: Address,
): SignerMerkleProof {
    const leaf  = computeSignerLeaf(signer);
    const index = tree.indexOf(leaf);

    if (index === -1) {
        throw new Error(
            `Signer ${signer} not found in tree. ` +
